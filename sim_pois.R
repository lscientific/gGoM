...
# save(R_list, T0_list, P0_list, file="./data/sim_pois_N=5J.RData")
...
# save(results, file="./data/sim_res_gom_Pois_N=5J.RData")
...
# save(results, file="./data/sim_res_nmf_Pois_N=5J.RData")


## results 
load("./data/sim_pois_N=5J.RData")
results_gom <- get(load("./data/sim_res_gom_Pois_N=5J.RData"))
results_nmf <- get(load("./data/sim_res_nmf_Pois_N=5J.RData"))
...
for(ni in 1:4) {
  ...
  for(rep in 1:n_rep) {
    ...
    print(res$t)
    t_res[rep, ni, 'NMF'] <- as.numeric(res$t)
    err_res[rep, ni, 'NMF'] <- sqrt(sum((R0 - res$R0_hat)^2)) / sqrt(N*J)
  }
}

err_dat <- as.data.frame.table(err_res)
err_dat$Method <- factor(err_dat$Method, levels=c('NMF', 'Proposed'))

p1 <- ggplot(data=err_dat, aes(x=N, y=Freq, fill=Method)) + 
...
t_dat <- as.data.frame.table(t_res)
t_dat$Method <- factor(t_dat$Method, levels=c('NMF', 'Proposed'))
df <- t_dat %>%
  group_by(N, Method) %>%
  summarise(
...
p2 <- ggplot(df, aes(N, t, color = Method, lty = Method)) +
...
## this figure corresponds to Figure 4
